// components/RedisExample.js
import React, { useState } from 'react';

function RedisExample(username) {
    ...
    const handleSetValue = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/setValue`, {
                ...
                body: JSON.stringify({"value": {statementText, tags}, "key": username}),
            });
  ...
  return (
    <div>
    <h1>RedisExample -- Set and Get Value from Redis</h1>
