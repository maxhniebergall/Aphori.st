...
const BLOCKED_PATH_PATTERNS: RegExp[] = [
  ...
  // Joomla
  /^\/(administrator|components|modules|plugins|libraries|installation)(\/|$)/i,
  ...
  /^\/(shell|c99|r57|b374k|b374|wso|alfa|alpha|cmd|mini|indoxploit|vodkas|404|bypass|
       config|function|filesman|filemanager|fm|filesbrowser|explorer|
       luna|dark|b4tm4n|priv8|pric|2604|dx|1nj3ct0r|itsecteam|
       spymasta|webshell|remvio)(\.php)?(\?.*)?$/i,
...
function blockIP(ip: string): void {
  ...
  // Async-persist to Redis via the API (best-effort, fire-and-forget)
  const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001';
  const secret = process.env.INTERNAL_API_SECRET ?? 'dev-internal-secret';
  fetch(`${apiUrl}/internal/block-ip`, {
    method: 'POST',
    headers: {
